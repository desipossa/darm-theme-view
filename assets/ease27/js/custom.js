...
$(function () {
    ...
    $(window).on('scroll', function () {
        ...
        if (sct > 90) {
            $('#header').addClass('scr')
            // 메인 오버레이 헤더일 때 : 상단 바를 위로 접어 넣는다
            $('#top_bar.overlay').addClass('scr_up')
        } else {
            $('#header').removeClass('scr')
            $('#top_bar.overlay').removeClass('scr_up')
        }
    }).trigger('scroll'); // 새로고침으로 중간에서 시작해도 상태를 맞춘다
...
$(function () {
    ...
    if (typeof AOS === 'undefined') {
        ...
        AOS.init({
            ...
            // 기본 700ms. 사진·격자·폼처럼 덩치가 큰 블록만 각 섹션 HTML 에서
            // data-aos-duration="1000" 으로 조금 느리게 두었습니다.
            duration: 700,
            offset: 80,
